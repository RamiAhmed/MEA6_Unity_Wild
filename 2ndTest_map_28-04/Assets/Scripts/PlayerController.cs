...
public class PlayerController : MonoBehaviour {
	...
	public bool bUseModifierKeysForRotations = true;
	...
	private CharacterMotor motorRef = null;
	...
	private void PlayStateLoop()
	{
		...
		if (Input.GetMouseButtonDown(0))
		{
			...
			if (UsedObject != null)
			{
				if (!bUseModifierKeysForRotations)
				{
				...
				else
				{
					if (!Input.GetKey(KeyCode.LeftControl)  && !Input.GetKey(KeyCode.RightControl) &&
						!Input.GetKey(KeyCode.LeftAlt)		&& !Input.GetKey(KeyCode.RightControl) &&
			...
			{				
				...
				else
				{
					...
					if (bUseModifierKeysForRotations)
					{
						...
						{
							LockMouseCursor();	
							motorRef.canControl = true;
						}
						else
						{
							UnlockMouseCursor();	
							motorRef.canControl = false;
						}
					}
				}
			}
	...
	private void HandleMainMenu()
	{
		...
		else if (GUILayout.Button("Fix Bugs"))
		{
			if (UsedObject != null)
			...
			if (bo != null && bo.enabled)
			{
				if (scenarioHandler.GetCurrentScenario() == ScenarioHandler.Scenario.KEYBOARD)
				{
					if (bUseModifierKeysForRotations)
						...
						rotationVector = HandleKeyboardRotation2();
				}
				else if (scenarioHandler.GetCurrentScenario() == ScenarioHandler.Scenario.MOUSE)
				{
					if (bUseModifierKeysForRotations)
						...
						rotationVector = HandleMouseRotation2();
				}
	...
	private Vector3 HandleMouseRotation2()
	{
		float rotationSpeedFactor = 3.0f;
		Vector3 rotationVector = Vector3.zero;
		bool bUnlockedMouse = false;
	...
	private Vector3 HandleKeyboardRotation2()
	{
		float rotationSpeedFactor = 3.0f;
		Vector3 rotationVector = Vector3.zero;
